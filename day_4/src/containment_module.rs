...
impl ProblemSolver for ProblemSolverPattern {
  ...
  fn initialize(lines: impl Iterator<Item = String>) -> Self::Input {
    ...
    for record in lines {
      let (left, right) = record.split_once(',').unwrap_or_else(|| panic!("precondition failure! lines must be ranges of positive integers of the form x-y,a-b, found (record):\n{}", 
        record));
      ...
      let left_range = (x.parse().expect(fmt_error), y.parse().expect(fmt_error));
      left_ranges.push(left_range);
      let (a, b) = right.split_once('-').expect(fmt_error);
      let right_range = (a.parse().expect(fmt_error), b.parse().expect(fmt_error));
